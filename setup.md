...
## 3. Create the `chest_x_rays_dev` Docker image

### GPU OPTION: Build our `chest_x_rays_dev` base Docker image

In the project repo, `cd` into the `orca_detector` directory:

```
cd ~/NIH-Chest-X-rays-Classification/src
```
...
sudo docker build \
    -t chest_x_rays_dev \
    -f Dockerfile.dev \
    --build-arg KAGGLE_USERNAME={YOUR_KAGGLE_USERNAME} \
    --build-arg KAGGLE_KEY={YOUR_KAGGLE_KEY} .
```
...
sudo docker run \
    ...
    -v ~/NIH-Chest-X-rays-Classification/results:/results \
    -p 8888:8888 \
    -p 4040:4040 \
    -p 32001:22 \
    chest_x_rays_dev
